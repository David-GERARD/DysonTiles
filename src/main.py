...
from shadows import create_no_shadow_grid_mask, create_full_shadow_grid_mask, create_cross_shadow_grid_mask
from shadows import DysonTiles


# TODO: find better way to match admin1 and admin2 names from API to geojson as fuzzywuzzy is not working well
# TODO: show the real count on log scale (instead of log10(count))
...
with col1:
    ...
    shadow_patern = st.selectbox(
        'Shadow Pattern',
        options=['No Shadow', 'Full Shadow', 'Cross Shadow'],
        index=0
    )
    ...
    opacity = st.slider('Opacity of activated tiles', min_value=0.0, max_value=1.0, value=0.5, step=0.01)
